...
class TestTaskRepository:
    ...
    """ Не прошел (many-to-many через association table)
    @pytest.mark.asyncio
    async def test_get_tasks_by_tag(self, db_session):
        \"""Тест фильтрации задач по тегу\"""
        user_repo = UserRepository(db_session)
        ...
        # Добавляем связь через association table напрямую
        from sqlalchemy import insert
        from app.models import task_tags  # Импортируйте association table
